...
if "bpy" in locals():
    import importlib
    importlib.reload(ot_add_move)
    importlib.reload(ot_copy_pos)
    importlib.reload(ot_generator)
    importlib.reload(ot_invert_pos)
    importlib.reload(ot_preview)
    importlib.reload(ot_remove_move)
    importlib.reload(properties)
else:
    from . import ot_add_move
    from . import ot_copy_pos
    from . import ot_generator
    from . import ot_invert_pos
    from . import ot_preview
    from . import ot_remove_move
...
def register():
    print('---------------------------------------------------------------')
    bpy.utils.register_class(properties.WalkCycleNewBones)
    bpy.utils.register_class(properties.WalkCycleSettings)

    bpy.types.Armature.aut_walk_cycle = bpy.props.PointerProperty(type=properties.WalkCycleSettings)
    bpy.types.Scene.awc_is_preview = bpy.props.BoolProperty()
    #bpy.app.handlers.frame_change_pre.clear()

    bpy.utils.register_class(ot_add_move.WalkCycleAddMove)
    bpy.utils.register_class(ot_remove_move.WalkCycleRemoveMove)
    bpy.utils.register_class(ot_copy_pos.WalkCycleCopyLocation)
    bpy.utils.register_class(ot_invert_pos.WalkCycleInvertValues)
    bpy.utils.register_class(ot_preview.WalkCyclePreview)
    bpy.utils.register_class(ot_generator.WalkCycleGenerate)
    bpy.utils.register_class(DATA_PT_auto_walk_cycle)

def unregister():
    bpy.utils.unregister_class(DATA_PT_auto_walk_cycle)
    bpy.utils.unregister_class(ot_generator.WalkCycleGenerate)
    bpy.utils.unregister_class(ot_preview.WalkCyclePreview)
    bpy.utils.unregister_class(ot_invert_pos.WalkCycleInvertValues)
    bpy.utils.unregister_class(ot_copy_pos.WalkCycleCopyLocation)
    bpy.utils.unregister_class(ot_remove_move.WalkCycleRemoveMove)
    bpy.utils.unregister_class(ot_add_move.WalkCycleAddMove)

    if ot_preview.frame_pre in bpy.app.handlers.frame_change_pre:
        bpy.app.handlers.frame_change_pre.remove(ot_preview.frame_pre)

    del bpy.types.Scene.awc_is_preview
    del bpy.types.Armature.aut_walk_cycle

    bpy.utils.unregister_class(properties.WalkCycleSettings)
    bpy.utils.unregister_class(properties.WalkCycleNewBones)
