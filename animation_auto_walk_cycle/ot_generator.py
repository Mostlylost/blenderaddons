...
def get_recursive_mat(bone):
    prts = bone.parent_recursive
    recurcive_mat = prts[-1].matrix.copy()
    for prt in reversed(prts[0:-1]):
        recurcive_mat *= prt.matrix

    return recurcive_mat
...
class WalkCycleGenerate(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if awc.anim:
            ...
            imat = (lf_dat.matrix if lf_dat.use_local_location else get_recursive_mat(lf_dat).inverted(),
                    rf_dat.matrix if rf_dat.use_local_location else get_recursive_mat(rf_dat).inverted())
            ...
            for c in range(cycles):
                ...
                op = (1-2*f)*openness
                loc0 = imat[f] * ((ant * dist * front_axis) + op * side_axis)
                loc1 = imat[f] * ((((2 * ant + 1)/2) * dist * front_axis) + amp * up_axis)
                loc2 = imat[f] * (((1 + ant) * dist * front_axis) + op * side_axis)
